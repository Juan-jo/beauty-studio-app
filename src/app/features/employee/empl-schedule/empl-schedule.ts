...
import { EmplBreak, EmplScheduleItem } from './model/empl-schedule.models';
...
export class EmplSchedule {
  ...
  editBreak(item: EmplScheduleItem, emplBreak?: EmplBreak) {
  
    let data: EditBreak = {
      id            : emplBreak?.id,
      startTime     : emplBreak?.startTime ?? '',
      endTime       : emplBreak?.endTime ?? '',
      dayOfWeek     : item?.dayOfWeek,
    ...
    const dialogRef = this.dialog.open(EmplEditBreak, {
      data: data
    });
